...
export class AuditService {
  static async log({
    ...
    entityType: string;
    entityId: number;
    action: 'CREATE' | 'UPDATE' | 'DELETE' | 'ASSIGN' | 'REPAIR' | 'REQUEST' | 'COMPLETE' | 'DAMAGE' | 'LOST' | 'LIQUIDATE' | 'PRINT' | 'CREATE_AND_COMPLETE' | 'IMPORT' | 'CANCEL' | 'UNDO' | 'LOGIN_SUCCESS' | 'LOGIN_FAILED' | 'LOGOUT' | 'CHANGE_PASSWORD' | 'RESET_PASSWORD' | 'LOCK_USER_BY_FAILED_LOGIN';
    details?: any;
    performedBy: string;
    tx?: any;
  }) {
  ...
  static async logAssetChange(assetId: number, oldData: any, newData: any, performedBy: string, tx?: any, reason?: string) {
    ...
    const fieldsToTrack = [
      'status', 'currentUserName', 'currentUserPhone', 'currentPosition', 'departmentName', 'locationName',
